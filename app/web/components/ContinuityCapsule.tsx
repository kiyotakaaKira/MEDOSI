...
const DEMO_DATA: CapsuleData = {
  bloodGroup: "O+",
  allergies: ["Penicillin"],
  chronicConditions: ["Hypertension"],
  activeMedications: [
    { name: "Amoxicillin", dosage: "500mg · 3× daily", reason: "Bacterial infection treatment" },
    { name: "Lisinopril", dosage: "10mg · daily", reason: "Blood pressure management" },
  ],
  historicalEvents: [
    { description: "Appendectomy", date: "2024-03-12", outcome: "Successful laparoscopic procedure" },
    { description: "Annual Physical", date: "2026-01-08", outcome: "All vitals within normal range" },
  ],
};
...
export function ContinuityCapsule() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:4000/api/ai/continuity-capsule/demo-patient")
      .then((res) => res.json())
      .then((data) => {
        if (data.success) setCapsuleData(data.capsule);
        else setCapsuleData(DEMO_DATA);
      })
      .catch(() => setCapsuleData(DEMO_DATA))
      .finally(() => {
        setLoading(false);
  ...
  const data = capsuleData ?? DEMO_DATA;
