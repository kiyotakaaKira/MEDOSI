        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-white/5 border-b border-white/10">
              {columns.map((col, i) => (
                <th key={i} className="p-4 text-xs font-medium text-white/60 uppercase tracking-wider">
                  {col.sortable ? (
                    <button
              ...
              currentData.map((item, rowIndex) => (
                <tr key={rowIndex} className="hover:bg-white/[0.02] transition-colors group">
                  {columns.map((col, colIndex) => (
                    <td key={colIndex} className="p-4 text-sm text-white/80 whitespace-nowrap">
